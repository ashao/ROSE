...
async def rose_mpi_ddict() -> None:
    ...
    d = DDict.attach(ddict_descriptor)
    ...
    async def training(*args, ddict_descriptor: str):
        ...
        d = DDict.attach(ddict_descriptor)
        dd_s = ddict.pickler(XPickler(), XPickler())

        iteration = dd_s["sim_meta_iter_count"] if "sim_meta_iter_count" in dd_s else 0
        iteration -= 1  # latest completed simulation

        X_parts, y_parts = [], []

        for rank in range(N_MPI_RANKS):
            X_parts.append(dd_s[f"sim_rank_{rank}_iter_{iteration}_X"])
            y_parts.append(dd_s[f"sim_rank_{rank}_iter_{iteration}_y"])
        ...
        d[f"model_iter_count"] = iteration + 1
        d[f"model_iter_{iteration}"] = gp
        d[f"mse_iter_{iteration}"] = mse
    ...
    async def active_learn(*args, ddict_descriptor: str):
        ...
        iteration = d["model_iter_count"] if "model_iter_count" in d else 0
        ...
        with ddict.pickler(XPickler(), XPickler()) as dd_s:
            dd_s[f"query_points_iter_{iteration}"] = X_candidates[top_idx].ravel()
        ...
        d["model_iter_count"] = iteration + 1
        d.detach()
    ...
    async def check_mse(*args, ddict_descriptor: str) -> float:
        ...
        mse: float = d[f"mse_iter_{iteration}"]
        print(
            f"[check]    iter={iteration} | MSE={mse:.6f} (threshold < {MSE_THRESHOLD})",
            flush=True,
        )
        d["mse_iter_count"] = iteration + 1
        d.detach()
        return mse
